...
class BlurContrastiveModelPair(nn.Module):
    ...
    def forward(self, seq, len_seq, targets, train_flag = True):
        ...
        for i in range(batch_size):
            ...
            neg_pair = torch.masked_select(neg_pair_, neg_pair_ < pos_pair_[-1] + margin)
            pos_pair = torch.masked_select(pos_pair_, pos_pair_ > neg_pair_[0] - margin)
            if len(neg_pair) < 1:
                continue

            pos_loss = torch.mean(pos_pair)
            neg_loss = torch.mean(margin-neg_pair)
